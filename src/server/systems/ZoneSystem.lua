--[=[
  Ghost Catcher Tycoon - Zone System
...
function ZoneSystem:new()
	...
	self.unlockedZones = {} -- UserId -> { zoneName: true/false }
	self.currencySystem = nil
	self.ghostSystem = nil
	self.bossActive = {} -- UserId -> { zoneName: { active, ghostId, startTime } }
	return self
end
...
function ZoneSystem:initializePlayer(player)
	...
	self.unlockedZones[userId]["Forest"] = true
...
function ZoneSystem:unlockZone(player, zoneName)
	...
	-- Unlock zone
	self.currencySystem:subtractEnergy(player, cost)
...
function ZoneSystem:defeatBoss(player, zoneName, ghostId)
	...
	-- Reward player
	local bossReward = 5000 -- Ecto-Energy
	local zoneConfig = Config.Zones[zoneName]
	if zoneConfig then
		bossReward = math.ceil(bossReward * zoneConfig.EnergyMultiplier)
	end

	if self.currencySystem then
		self.currencySystem:addEnergy(player, bossReward)
	end
