...
-- Try to load DataStore, but fall back to in-memory for Studio testing
local success, err = pcall(function()
	playerDataStore = DataStoreService:GetDataStore(Constants.DataStores.PlayerData)
end)

if not success then
	warn("DataStore not available (expected in Studio): " .. tostring(err))
	warn("Using in-memory storage for testing only")
	playerDataStore = nil -- Will use cache only
end
